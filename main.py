...
def load_input(day: int) -> Input:
    return Input(f"input/input_day{day}.txt")


def solve_day(day: int):
    print(
        f"{ANSI_ESC_INFO}Preparing solution {day:02} - {PUZZLES[day - 1]}{ANSI_ESC_CLOSE}"
    )
    input = load_input(day)
    module = importlib.import_module(f"solution.day{day:02}")
